...
<div class="container-fluid">
    <div class="card shadow-sm">
        <div class="card-header">
            <h6 class="m-0 font-weight-bold text-primary">{{ __('Étape 1 sur 2 : Veuillez choisir un livre') }}</h6>
        </div>
        <div class="card-body">
            <!-- Search Form -->
            <form action="{{ route('teacher.quizzes.select-book') }}" method="GET" class="mb-4">
                <div class="input-group">
                    <input type="text" name="search" class="form-control" placeholder="{{ __('Rechercher un livre par titre...') }}" value="{{ $search ?? '' }}">
            ...
            @if($books->isEmpty())
                <div class="text-center py-5">
                    <p class="text-muted">{{ __('Aucun livre trouvé.') }}</p>
                </div>
            @else
                <div class="row">
