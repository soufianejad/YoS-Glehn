...
@section('content')
<div class="container">
    <h1>{{ __('Monthly Payment Report') }} ({{ now()->format('F Y') }})</h1>

    <div class="card mb-4">
        <div class="card-header">{{ __('Summary') }}</div>
        <div class="card-body">
            <p><strong>{{ __('Total Monthly Revenue:') }}</strong> ${{ number_format($totalMonthlyRevenue, 2) }}</p>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="6">{{ __('No payments recorded this month.') }}</td>
                </tr>
            @endforelse
        </tbody>
    </table>
