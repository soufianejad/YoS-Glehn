<!-- resources/views/library/popular.blade.php -->

@extends('layouts.app')

@section('content')
<div class="container">
    <h1>{{ __('Popular Books') }}</h1>

    <div class="row mb-3">
        <div class="col-md-12">
            <form action="{{ route('library.popular') }}" method="GET" class="form-inline">
            ...
            <ul class="list-group">
                @foreach($categories as $category)
                    <li class="list-group-item">
                        <a href="{{ route('library.index', ['category' => $category->slug]) }}">{{ $category->name }}</a>
                    </li>
                @endforeach
            </ul>
        </div>
