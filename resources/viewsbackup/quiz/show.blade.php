...
@section('title', __('Quiz for :book', ['book' => $book->title]))
@section('header', __('Quiz: :book', ['book' => $book->title]))
...
<div class="container">
    ...
    <form action="{{ route('quiz.submit', $book) }}" method="POST">
        ...
        @foreach($quiz->questions as $question)
            <div class="card mb-3">
                <div class="card-header fw-bold">{{ __('Question') }} {{ $loop->iteration }}</div>
                <div class="card-body">
        ...
        <div class="text-end mt-4">
            <button type="submit" class="btn btn-primary btn-lg">{{ __('Soumettre le Quiz') }}</button>
        </div>
    </form>
</div>
@endsection
