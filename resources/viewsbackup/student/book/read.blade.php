...
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-md-12">
            <h1>{{ $book->title }}</h1>
            <p class="lead">{{ __('par') }} {{ $book->author->name ?? 'N/A' }}</p>
            <p>{{ $book->description ?? __('No description available.') }}</p>
            <hr>
        </div>
    </div>
    ...
    <div class="row mb-4">
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">{{ __('Détails du livre') }}</div>
                <div class="card-body">
                    <p><strong>{{ __('Catégorie:') }}</strong> {{ $book->category->name ?? 'N/A' }}</p>
                    <p><strong>{{ __('Niveau:') }}</strong> {{ $book->level ?? 'N/A' }}</p>
                    <p><strong>{{ __('Pages:') }}</strong> {{ $book->pdf_pages ?? 'N/A' }}</p>
                    <p><strong>{{ __('Publié:') }}</strong> {{ $book->published_at ? $book->published_at->format('M d, Y') : 'N/A' }}</p>
                    <p><strong>{{ __('ISBN:') }}</strong> {{ $book->isbn ?? 'N/A' }}</p>
                    <p><strong>{{ __('Langue:') }}</strong> {{ $book->language ?? 'N/A' }}</p>
                </div>
            </div>
        </div>
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    <a href="{{ route('student.book.show', $book) }}" class="btn btn-info btn-block">{{ __('Retour aux détails du livre') }}</a>
                    @if($book->audio_file)
                        <a href="{{ route('student.book.listen', $book) }}" class="btn btn-primary btn-block mt-2">{{ __('Écouter l\'audio') }}</a>
                    @endif
                    @if($book->pdf_file)
                        <a href="{{ route('student.book.download', $book) }}" class="btn btn-success btn-block mt-2">{{ __('Télécharger le PDF') }}</a>
                    @endif
                    @if($book->has_quiz)
                        <a href="{{ route('student.quiz.book-quiz', $book) }}" class="btn btn-warning btn-block mt-2">{{ __('Passer le Quiz') }}</a>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="col-md-12">
            @if($book->pdf_file)
                ...
                <div class="mt-2 d-flex justify-content-between align-items-center">
                    <div>
                        <button id="prev" class="btn btn-secondary">{{ __('Précédent') }}</button>
                        <button id="next" class="btn btn-secondary">{{ __('Suivant') }}</button>
                    </div>
                    <span>{{ __('Page') }}: <span id="page-num">1</span> / <span id="page-count">0</span></span>
                </div>
            @else
                <p>{{ __('Aucun fichier PDF disponible pour ce livre.') }}</p>
            @endif
        </div>
    </div>
</div>
